...
def get_description(url):
    ...
    if response.status_code == 200:
        ...
        label.configure(text=desc[0].text.replace('\n', "").replace('\t', '').replace('\r', ''))
...
for elem in result.keys():
    button = customtkinter.CTkButton(scrollable_frame_left, text=elem, command=lambda url=result[elem]: get_description(url), width=280, height=50)
...
label = customtkinter.CTkLabel(scrollable_frame_right, text="", wraplength=560,  justify="left", font=("Times New Roman", 18))
